...
static std::string GetFP8Type(DataType type) {
  ...
  if (type.is_float8_e4m3fn() || type.is_float8_e4m3fnuz() || type.is_float8_e4m3()) {
    stream << "fp8_e4" << vec << "_t";
  } else if (type.is_float8_e5m2() || type.is_float8_e5m2fnuz() || type.is_float8_e5m2()) {
    stream << "fp8_e5" << vec << "_t";
  } else {
    LOG(FATAL) << "Unsupported FP8 type in CUDA codegen " << type;
  }
  return stream.str();
}
...
void CodeGenTileLangCUDA::VisitExpr_(const CallNode *op, std::ostream &os) {
  ...
  if (op->op.same_as(builtin::ptx_cp_async())) {
    ...
    auto index_map_func = ffi::Function::GetGlobal(
