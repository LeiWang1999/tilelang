...
def test_lazy_jit_accepts_trailing_dynamic_symbol_values(cache_dirs):
    import tilelang
    import tilelang.language as T
    ...
    def make_kernel(block_size: int, feature_dim: int):
        tensor_stride = T.dynamic("tensor_stride")

        @T.prim_func
        def copy_kernel(
            src: T.StridedTensor((block_size, feature_dim), (tensor_stride, 1), "uint8"),
            dst: T.StridedTensor((block_size, feature_dim), (tensor_stride, 1), "uint8"),
        ):
            with T.Kernel(block_size, threads=128) as i:
                for j in T.Parallel(feature_dim):
                    dst[i, j] = src[i, j]

        return copy_kernel

    kernel = make_kernel(130, 584, 584)

    assert [tuple(map(str, param.shape)) for param in kernel.adapter.params] == [("130", "584"), ("130", "584")]
